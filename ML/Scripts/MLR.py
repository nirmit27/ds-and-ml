...
# Graphical Representation [Univariate analysis]
...
plt.hist(cars.MPG)
plt.boxplot(cars.MPG) # No outliers


# Jointplot [Bivariate analysis]
sns.jointplot(x=cars['HP'], y=cars['MPG']) #  ->  Curvilinear relationship
...
# Row index 76 is showing high influence so we can exclude that entire row
...
# Summary
ml_new.summary() # -> Value of R-squared INCREASED
...
Vif_frame = pd.DataFrame({'Variables':['HP', 'WT', 'VOL', 'SP'], 'VIF':[vif_hp, vif_wt, vif_vol, vif_sp]})
Vif_frame
